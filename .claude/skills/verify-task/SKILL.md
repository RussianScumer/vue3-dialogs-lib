...
Runs both projects (`unit` jsdom, `browser` Playwright Chromium). If the browser project fails to launch rather than fails a test, tell the user to run `pnpm exec playwright install chromium`; that is not a test failure. If an existing spec fails, that is constraint §6 territory: raise it, do not edit the assertion.
